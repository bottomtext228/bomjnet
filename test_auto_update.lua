...
function main()
    ...
    sampAddChatMessage('¬ас€ хуесос бл€€€€€€€', -1)
    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if tonumber(updateIni.updatefile.vers) > script_vers then
                sampAddChatMessage("≈сть обновление! ¬ерси€: " .. updateIni.updatefile.vers_text, -1)
                update_state = true
            end
	...
	while true do
        ...
        if update_state then
            downloadUrlToFile(script_url, script_path, function(id, status)
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    sampAddChatMessage("¬ас€, ты обречЄн на погибель.....", -1)
                    thisScript():reload()
                end
            end)
            break
...
function cmd_update(arg)
 sampAddChatMessage('√де обнова бл€€€€€€', -1)
