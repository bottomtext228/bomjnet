	...
	while true do
        ...
        if update_state then
            downloadUrlToFile(script_url, script_path, function(id, status)
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    sampAddChatMessage("ВАСЯ ХУЕСОС ЕБАНЫЙ УЕБАН НАХУЙ MQMQMQMQMMQMQMMQ", -1)
                    thisScript():reload()
                end
            end)
            break
...
function cmd_update(arg)
    sampAddChatMessage('Обнова ебать произошла', -1)
    sampAddChatMessage('Да внатуре пиздец ебаа', -1)
end